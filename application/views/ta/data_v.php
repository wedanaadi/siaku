        ...
        <div class="col-12 col-md-12 col-lg-12">
          <div class="card">
            <div class="card-header">
              <button class="btn btn-icon icon-left btn-primary" id="ta_t">
            ...
            <div class="card-body">
              <div class="table-responsive">
                <table class="table table-striped" id="ta_tabel" style="width:100%">
                  <thead>
                    <tr>
                      <th>
                        #
...
<script>
  ...
  var tabel = $("#ta_tabel").dataTable({
    ...
    serverSide: true,
    ajax: {
      "url": "<?php echo base_url() . 'TahunAjaran_c/ignited_data' ?>",
      "type": "POST"
    },
    columns: [{
        "data": null,
  ...
  $(document).on('click', '#ta_t', function() {
    isSave = 1
    $.ajax({
      type: "GET",
      dataType: "JSON",
      url: "<?= base_url(); ?>TahunAjaran_c/form/",
  ...
  $('#ta_f').on('submit', function(e) {
    ...
    if (form[0].checkValidity() === false) {
      ...
      $.ajax({
        ...
        url: isSave ? '<?= base_url() ?>TahunAjaran_c/create' : '<?= base_url() ?>TahunAjaran_c/update/' + idta,
        success: function(respon) {
          tabel.api().ajax.reload();
          setTimeout(function() {
            $('#modal').modal('hide');
            notifsukses('Tahun Ajaran', isSave ? 'disimpan' : 'diubah');
          }, 200);
        }
      });
    }
  ...
  $(document).on('click', '#ta_u', function() {
    isSave = 0;
    const id = $(this).attr('id-pk');
    $.ajax({
      type: "GET",
      dataType: "JSON",
      url: "<?= base_url(); ?>TahunAjaran_c/form/" + id,
      success: function(respon) {
        $('#ta_f')[0].reset();
        $('#ta_f').removeClass('was-validated');
        $('#submit').removeClass('disabled btn-progress');
        $("#tag_html").html(respon.view);
        $('[name=tahunajaran]').val(respon.data.tahun_ajaran);
        $('[name=semester]').val(respon.data.semester);
        idta = respon.data.id;
  ...
  $(document).on('click', '#ta_d', function() {
    isSave = 0;
    const id = $(this).attr('id-pk');
    swal({
        title: 'Hapus data ?',
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            type: "POST",
            url: "<?= base_url() ?>TahunAjaran_c/hapus/" + id,
            success: function(respon) {
              tabel.api().ajax.reload();
              notifsukses('Tahun Ajaran', 'dihapus');
            }
          });
        } else {
          notifgagal('Tahun Ajaran');
        }
      });
  });
</script>